...
- Name: image alt text validation
- Location: /home/connerohnesorge/Documents/001Repos/conneroh.com.git/feat/verifiable/tests/browser/accessibility.test.ts:14:1
...
Received: null
    at /home/connerohnesorge/Documents/001Repos/conneroh.com.git/feat/verifiable/tests/browser/accessibility.test.ts:25:19
```
...
```ts
   ...
   2 |
   3 | test('heading hierarchy validation', async ({ page }) => {
   4 |   await page.goto('/')
   5 |   
   6 |   // Test heading hierarchy
   7 |   const headings = await page.locator('h1, h2, h3, h4, h5, h6').count()
   8 |   expect(headings).toBeGreaterThan(0)
   9 |   
  10 |   // Check if h1 exists
  11 |   await expect(page.locator('h1')).toBeVisible()
  12 | })
  13 |
  14 | test('image alt text validation', async ({ page }) => {
  15 |   await page.goto('/')
  16 |   
  17 |   // Check that all images have alt text with debug output
  18 |   const images = await page.locator('img').all()
  19 |   for (let i = 0; i < images.length; i++) {
  20 |     const img = images[i]
  21 |     const alt = await img.getAttribute('alt')
  22 |     const src = await img.getAttribute('src')
  23 |     
  24 |     try {
> 25 |       expect(alt).toBeTruthy()
     |                   ^ Error: expect(received).toBeTruthy()
  26 |     } catch (error) {
  27 |       console.log(`Image ${i} with src "${src}" is missing alt text. Alt attribute value: "${alt}"`)
  28 |       throw error
  29 |     }
  30 |   }
  31 | })
  32 |
  33 | test('form label associations', async ({ page }) => {
  34 |   await page.goto('/')
  35 |   
  36 |   // Check contact form if it exists
  37 |   const contactSection = page.locator('#contact')
  38 |   if (await contactSection.isVisible()) {
  39 |     const inputs = await contactSection.locator('input, textarea, select').all()
  40 |     for (const input of inputs) {
  41 |       const id = await input.getAttribute('id')
  42 |       if (id) {
  43 |         await expect(page.locator(`label[for="${id}"]`)).toBeVisible()
  44 |       }
  45 |     }
  46 |   }
  47 | })
  48 |
  49 | test('landmarks and semantic structure', async ({ page }) => {
  50 |   await page.goto('/')
  51 |   
  52 |   // Test landmarks exist
  53 |   await expect(page.locator('header')).toBeVisible()
  54 |   await expect(page.locator('main')).toBeVisible()
  55 |   await expect(page.locator('footer')).toBeVisible()
